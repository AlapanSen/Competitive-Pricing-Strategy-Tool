...
# Configure logging with more flexible path handling
log_dir = os.path.join(os.getcwd(), 'logs')
os.makedirs(log_dir, exist_ok=True)
log_file = os.path.join(log_dir, "pricing_strategy.log")
...
# Create directories for outputs with more flexible path handling
pricing_dir = os.path.join(os.getcwd(), 'pricing_strategies')
viz_dir = os.path.join(os.getcwd(), 'visualizations', 'pricing_strategies')
...
class PricingStrategy:
    ...
    def __init__(self, 
        ...
        """
        # Use a more flexible approach to model directory
        if models_dir is None:
            # Default to models/improved/category_models relative to current directory
            self.models_dir = os.path.join(os.getcwd(), 'models', 'improved', 'category_models')
        else:
    ...
    def load_category_benchmarks(self, file_path=None):
        ...
        """
        # Use a more flexible approach to file path
        if file_path is None:
            file_path = os.path.join(os.getcwd(), 'logs', 'outlier_stats.json')
        ...
        try:
            if os.path.exists(file_path):
                ...
                # If the file doesn't exist, try to create default benchmarks
                logger.warning(f"Benchmark file not found: {file_path}, creating default benchmarks")
                self._create_default_benchmarks()
                return True
        except Exception as e:
            logger.error(f"Error loading category benchmarks: {str(e)}")
            # Create default benchmarks on error
            self._create_default_benchmarks()
            return False
            
    def _create_default_benchmarks(self):
        """Create default benchmarks if file cannot be loaded"""
        default_categories = [
        ...
        for category in default_categories:
            ...
            if category == 'Computers':
                ...
                q1, q3 = 150, 450  # Adjusted to more realistic values
            ...
            self.category_benchmarks[category] = {
                ...
                'q3': float(q3),
                'iqr': float(iqr),
                'lower_bound': float(max(0, q1 - 1.5 * iqr)),
    ...
    def predict_price(self, product_features, category):
        ...
        if hasattr(model, 'feature_names_in_'):
            ...
            # Make prediction - no fallback, either works or fails
            predicted_value = model.predict(features_array)[0]
        else:
            # For models without feature_names_in_
        ...
        if category in self.calibration_factors:
            calibration_factor = self.calibration_factors[category]
        ...
        # Make sure we have benchmarks for this category
        if category not in self.category_benchmarks:
            logger.warning(f"No benchmarks found for category: {category}, creating default benchmarks")
            self._create_default_benchmarks()
